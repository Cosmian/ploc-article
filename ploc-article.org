...
#+latex_header: \setminted{style=bw}
...
#+attr_latex: :options [Adaptive Security]
#+begin_definition
...
In  order  to   store  and  encrypt  its  data,  $\ploc$   relies  on  an  \((s,
D)\)-binary-tree encryption scheme which, for the sake of this article, is based
on the abstract data type defined as follows:
#+attr_latex: :options [\((s, D)\) Binary Tree]
#+begin_definition
An \((s, D)\) binary tree is an  abstract data type parameterized by a node size
$s$ and a depth  $D$, which valid states are the binary trees  of depth $D$ with
nodes  that are  isomorphic  to \(s\)-long  byte strings  (i.e.  tree nodes  are
serializable  to  \(s\)-long  byte  strings), and  that  exposes  the  following
operations:
- $\Select(\{b_i\})$ that returns the subtree defined by the branches $\{b_i\}$;
- $\Mutate(\{b_i\},  tx)$  that mutates  the  subtree  defined by  the  branches
  $\{b_i\}$ by replacing it by its image by the transformation $tx$ on it, which
  should be a function  that takes as argument a tree and returns  a tree of the
  same shape.
#+end_definition

Where a /branch/ is defined as a  bit string representing the choices to be made
at each level between  the left and right children of a  node.  Such bit strings
are encoded as integers  in which the choice made at the root  is encoded by the
bit of  exponent zero.  This choice  of representation allows for  efficient and
simple tree traversals.  The number of bits in a branch needs not be smaller the
tree depth  since the additional  bits can simply  be ignored by  the traversal.
This allows using the output of ~H~ as branch.
#+name: branch-utils
#+begin_src scheme
...
An  \((s, D)\)-binary-tree  encryption  scheme therefore  exposes the  following
operations:
- ~(tree-setup)~ creates a  server-side encrypted complete binary  tree of depth
  ~D~ and returns a freshly generated key;
- ~(tree-select  key  branches)~  returns  the  subtree  defined  by  the  given
  branches;
- ~(tree-mutate  key subtree  tx)~  mutates  the subtree  defined  by the  given
  branches using the given transformation.
Additionally, we suppose in the following sections that the subtrees returned by
this scheme are made of nodes that  are either a vector containing the node data
and its left and right children and sentinel nodes represented by the empty list
are used to make processing the tree easier.
...
The   setup  simply   consists  in   calling   the  setup   of  the   underlying
\((s,D)\)-binary-tree encryption scheme:
#+name: ploc-setup
#+begin_src scheme
...
value, and a branch called /target/.
#+name: datum
#+begin_src scheme
...
The compaction  is performed  on a  subtree of $n$  branches that  are generated
using  a  procedure  of  no  argument called  a  /scheduler/.   Some  scheduling
strategies  are better  than others:  if the  same set  of branches  were to  be
constantly scheduled  for compaction,  the datums which  target branches  have a
small intersection with  this set of branches would accumulate  near the root of
the  tree, eventually  causing an  overflow.  We  therefore choose  a scheduling
strategy that sweeps the  tree branches in a regular and  uniform way: the lower
bits are all covered by at least one  of the branches and each branch covers the
remaining bits in a circular fashion.
#+begin_export latex
For example with $D=3$ and $n=2$, the two upper-most levels representing the two
...
\\
\end{tabular}
\caption{Scheduled subtrees for $D=3$ and $n=2$.\label{fig:scheduling}}
\end{figure}
#+end_export
#+begin_export html
<p>For example with \(D=3\) and  \(n=2\), the two upper-most levels representing
  ...
  (define (take n xs)
    (let loop ((n n) (xs xs) (taken (list)))
      (if (or (zero? n) (null? xs))
          (values taken xs)
          (loop (- n 1) (pair (head xs) taken) (tail xs)))))
#+end_src
...
#+begin_src scheme :noweb-ref 'nil
  (define (split-with pred? xs)
    (let loop ((xs xs) (lhs (list)) (rhs (list)))
      (match xs
        ((x . xs) (if (pred? x)
                      (loop xs (pair x lhs) rhs)
                      (loop xs lhs (pair x rhs))))
        (()       (values lhs rhs)))))
#+end_src
#+name: compact
#+begin_src scheme
...
#+begin_src scheme
  (define (get-all-targets key label)
    (map (λ (v) (get-target key label v))
         (iota V)))
#+end_src
and retrieves the data targeting the same target branches:
#+name: ploc-search
#+begin_src scheme
    ...
    (let* ((targets (get-all-targets key label))
      ...
      (reverse
       (foldl (λ (acc target)
              ...
              targets))))
#+end_src
where ~tree-data~  is the procedure using a depth-first search:
#+name: tree-dfs
#+begin_src scheme
  (define (tree-dfs f acc root)
    (let loop ((node root) (acc acc))
      (match node
        ...
        (() acc))))
#+end_src
to return a multi-map containing all values stored in the given tree, indexed by
their target:
#+name: tree-data
#+begin_src scheme
...
In this section, we assume  the existence of an \(\cL_{\tau}\)-adaptively-secure
\((s, D)\)-binary-tree encryption scheme, where $\cL_\tau$ is defined as follow:
- $\cL_{\tau}(\Setup) = \Setup$;
- $\cL_{\tau}(\Select, \{b_i\}) = (\Select, \{b_i\})$;
- $\cL_{\tau}(\Mutate, \{b_i\}, tx) = (\Mutate, \{b_i\})$.
That is,  these binary-tree-encryption-scheme operations only  leak the branches
defining the subtree they are working on.   Such a scheme is actually trivial to
design  as   it  only  requires  nodes   to  be  encrypted,  and   one  possible
implementation is described in appendix [[*Tree Encryption Scheme]].

#+begin_theorem
Given  an  \(\cL_{\tau}\)-adaptively-secure  \((s,  D)\)-binary-tree  encryption
scheme    with     public    parameters     $pp_{\tau}$,    $\ploc$     is    an
\(\cL_{\pi}\)-adaptively-secure \((\bbL,\bbV)\)-multi-map encryption scheme with
public parameters $(pp_{\tau},  c, n, B, V)$, where $s  = c\lg|\bbV|$, $D=\lg B$
and $\cL_{\pi}$ is defined as follows:
- $\cL_{\pi}(\Setup) = \Setup$
...
V)$, and an \(\cL_{\tau}\)-adaptively-secure \((c\lg|\bbV|, \lg B)\)-binary-tree
encryption scheme  with public  parameters $pp_\tau$,  there exists  a simulator
...
Note that  in the implementation of  the binary-tree encryption scheme  given in
appendix [[*Tree Encryption  Scheme]], the only  public parameter is  the encryption
overhead.

#+begin_corollary
Given  an  \(\cL_{\tau}\)-adaptively-secure  \((s,  D)\)-binary-tree  encryption
scheme  with  public  parameters  $pp_{\tau}$, $\ploc$  is  an  injection-secure
\((\bbL,\bbV)\)-multi-map encryption scheme  with public parameters $(pp_{\tau},
c, n, B, V)$, where $s = c\lg|\bbV|$, $B = \lg B$.
#+end_corollary
...
the fetched  tree and in which  the calls to ~get-target~  and ~get-all-targets~
have been inlined:
...
-    (reverse
-     (foldl (λ (acc target)
-              (match (multi-map-get values target)
-                (()      acc)
-                ((value) (cons value acc))
-                ( values (error ploc-search
-                                "more than one values found"
-                                target values))))
-            (list)
-            targets))))
+  (let* ((targets (map (λ (v) (H key label v)) (iota V)))
+         (subtree (tree-select key targets)))
+    (values)))
#+end_src
which gives:
...
adversarial advantage by a call to a PRF:
#+begin_src sh :exports results :results output :wrap src diff
...
#+begin_src scheme :tangle tmp/ploc-search-3.scm
  (define (ploc-search key label-idx)
    (let* ((targets (map (λ (v) (PRF label-idx v)) (iota V)))
      ...
      (values)))
#+end_src
Finally,  by  security  of  the  binary-tree  encryption  scheme,  the  call  to
~tree-select~ can  be replaced by  a call to $\phi_{pp_\tau}  \circ \cL_{\tau}$.
Since  $\cL_{\tau}(\Fetch,  \mathrm{targets})   =  (\Fetch,  \mathrm{targets})$,
~(tree-select key targets)~ is indistinguishable from ~(phi 'select targets)~:
#+begin_src sh :exports results :results output :wrap src diff
...
error and does not modify the volumes:
#+begin_src sh :exports results :results output :wrap src diff
...
#+begin_src scheme :tangle tmp/ploc-insert-1.scm :exports none
  ...
  (define (ploc-insert key label value)
    (let ((data (list (make-datum key label value)))
      ...
      (values)))
#+end_src
By  security of  the  tree  encryption scheme,  the  call  to ~(tree-mutate  key
(scheduler) tx)~ can  be replaced with only negligible  adversarial advantage by
...
\end{array}\]
#+end_export
In what follows, we will be interested  in finding the smallest integer value of
$k$ such that $m  \times P(X \ge k) \le 2^{-\lambda}$  for some multiplicity $m$
which value will depend on the proof. We therefore pose:
#+begin_export latex
\[\begin{equation}
...
Therefore,  a tree  overflow can  only happens  if  the root  is the  tip of  an
...
#+caption: $c^*(\lambda, n, N)$ in function of $n$ and $D$ for $\lambda = 128$.
|   / |  < |     |    |
| $n$ |    | $D$ |    |
|     | 16 |  32 | 40 |
|-----+----+-----+----|
...
#+caption: $c^*(\lambda, n, N)$ in function of $n$ and $D$ for $\lambda = 64$.
|   / |  < |     |    |
| $n$ |    | $D$ |    |
|     | 16 |  32 | 40 |
|-----+----+-----+----|
...
#+begin_theorem
When  instantiated using  an \((c  \lg  \bbV, B)\)-tree  encryption scheme  with
...
implementation  described in  section [[*$\ploc$]].   In  the case  of an  $\Insert$
...
Theorem [[theorem:performance]]  guarantees  logarithmic  performances  in  $B$  for
...
#+caption: Operation performances ($\Search \quad \Insert$) in function of $n$ and $B$ for $V = \sqrt{B}$ and $c$ given in Table [[table:numerical-analysis-lambda=128]].
| /          | <                         |                          |                           |
| $n$ \\ $B$ | $2^{10}$                  | $2^{16}$                 | $2^{20}$                  |
|------------+---------------------------+--------------------------+---------------------------|
...
The scheme described  in section [[*$\ploc$]] only supports  $\Insert$ and $\Search$
queries.  However, the  possibility  to  delete bindings  is  usually needed  by
concrete  use-cases.   It is  actually  possible  to implement  a  fully-dynamic
...
$\ploc$  relies  on an  \((s,  D)\)-binary-tree  encryption scheme,  defined  in
section [[*$\ploc$]]. The present section describes an implementation of this scheme
for completeness only.

** Tree serialization

Following the design choices discussed  in [cite:@EPRINT:BreHeb24], we rely on a
server abstraction  exposing a  linear 128-bit memory  space with  the following
operations:
...
an address that is  computed from its /node ID/ -- defined  as the shared prefix
of each branch leading to this node -- and depth as follows:
#+name: make-address
#+begin_src scheme
  (define (make-address depth target)
    (G depth (node-id depth target)))
#+end_src
where  ~G~ is  a hash  function which  is only  required to  have a  good-enough
collision resistance.

** Setup

The ~tree-setup~  simply consists  in setting-up a  new server-side  memory, and
returning a fresh AE key.
#+name: tree-setup
#+begin_src scheme
  (define (tree-setup)
    (memory-setup)
    (AE-keygen))
#+end_src
Not initializing the  tree avoids a $O(N)$ setup without  leaking any additional
information in the  persistent-adversary model. Note that  we could pre-allocate
enough server-side memory up-front since the size of the tree is already known.

** Select

To return the subtree  generated by the given target branches,  we first need to
derive  all  the addresses  of  its  nodes, which  can  be  performed using  the
following ~subtree-addresses~ procedure:
            ...
            (loop (+ depth 1) rhs
                  (loop (+ depth 1) lhs
                        (pair (make-address depth (car branches))
                              addresses)))))))
#+end_src
Reading data from  a word is performed by decrypting  this word and interpreting
the resulting  bytes as a  serialized data.  Since  the tree was  no initialized
in-memory upon  setup, some addresses  might be free,  which must be  taken into
account:
#+name: word->data
#+begin_src scheme
...
#+end_src
Finally, the tree can  be built from the words read by relying  on the fact that
~memory-read~ preserves  the address order:  we can perform the  exact /reverse/
tree traversal as  in ~subtree-addresses~ (in which the right  recursive call is
performed first while it was performed last in ~subtree-address~) since the list
in which the addresses were accumulated is a stack.
#+name: words->tree
#+begin_src scheme
      ...
      (if (or (null? branches) (< max-depth depth))
          ...
          (let*-values
            ...
            (values (new-node (word->data AE (car words))
                    ...
                    (cdr words))))))
#+end_src
We can now write the complete  ~tree-select~ procedure:
#+name: tree-select
#+begin_src scheme
...
Mutating the tree consists in fetching the subtree defined by the given branches
much like  the ~select~ procedure,  generating the  new subtree using  the given
transformation and  writing it  back to  memory.  To  do that,  we first  need a
procedure to convert a tree into bindings:
#+name: tree->bindings
#+begin_src scheme
